...
new #[Layout('components.layouts.lecturer')] class extends Component {
    ...
    public function assignRoles(): void
    {
        if ($this->lecturer) {
            ...
            $this->redirect(route('lecturer.assignments.index'), navigate: true);
        }
    }
};
...
<div>
    <section class="w-full">
        <div
            class="w-full max-w-5xl mx-auto bg-white dark:bg-zinc-900 rounded-lg shadow-xl border border-zinc-200 dark:border-zinc-700 p-6 sm:p-8">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                ...
                <div class="flex items-center gap-2">
                    <a href="{{ route('lecturer.assignments.index') }}"
                        class="bg-zinc-200 dark:bg-zinc-700 text-zinc-800 dark:text-zinc-200 px-5 py-2 rounded-lg hover:bg-zinc-300 dark:hover:bg-zinc-600 transition-colors whitespace-nowrap"
                        wire:navigate>
                        Back
                    </a>
            ...
            <p class="text-zinc-600 dark:text-zinc-400 mb-6 text-sm">
                Select the roles that this lecturer should have. Their permissions will be automatically updated based
            ...
            @if (session('success'))
                <div
                    class="mb-6 p-4 bg-green-100 border border-green-400 text-green-700 dark:bg-green-900 dark:border-green-700 dark:text-green-200 rounded-lg">
                    {{ session('success') }}
                </div>
            @endif
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    @forelse ($allRoles as $role)
                        <label
                            ...
                            <span
                                class="ml-3 text-sm sm:text-base text-zinc-800 dark:text-zinc-300 font-medium">{{ $role->name }}</span>
                        </label>
                    @empty
